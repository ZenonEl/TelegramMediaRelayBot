...
public class SqliteInboxRepository(IDbConnection dbConnection) : IInboxRepository
{
    public async Task<long> AddItemAsync(int ownerUserId, int fromContactId, string caption, string payloadJson, string status)
    {
        string sql = @"INSERT INTO InboxItems (OwnerUserId, FromContactId, Caption, PayloadJson, Status)
                    VALUES (@ownerUserId, @fromContactId, @caption, @payloadJson, @status);
                    SELECT last_insert_rowid();";
        long id = await dbConnection.ExecuteScalarAsync<long>(sql, new { ownerUserId, fromContactId, caption, payloadJson, status });
        return id;
    }

    public async Task<bool> SetStatusAsync(long inboxItemId, string status)
    {
        string sql = "UPDATE InboxItems SET Status=@status WHERE ID=@id";
        int affected = await dbConnection.ExecuteAsync(sql, new { id = inboxItemId, status });
        return affected > 0;
    }

    public async Task<IEnumerable<InboxItemDto>> GetItemsAsync(int ownerUserId, int limit = 20, int offset = 0)
    {
        string sql = @"SELECT ID as Id, OwnerUserId, FromContactId, Caption, PayloadJson, Status, CreatedAt
    ...
    {
        List<string> conditions = new List<string> { "OwnerUserId=@ownerUserId" };
    ...
    {
        int affected = await dbConnection.ExecuteAsync("DELETE FROM InboxItems WHERE ID=@id", new { id });
        return affected > 0;
    }

    public async Task<int> GetNewCountAsync(int ownerUserId)
    {
        string sql = "SELECT COUNT(*) FROM InboxItems WHERE OwnerUserId=@ownerUserId AND Status='new'";
    ...
    {
        string sql = "UPDATE InboxItems SET Status=@toStatus WHERE OwnerUserId=@ownerUserId AND Status=@fromStatus" + (fromContactId.HasValue ? " AND FromContactId=@fromContactId" : "");
    ...
    public async Task<int> DeleteForOwnerAsync(int ownerUserId, string? statusFilter = null, int? fromContactId = null)
    {
        ...
        if (fromContactId.HasValue) conditions.Add("FromContactId=@fromContactId");
        string where = string.Join(" AND ", conditions);
        string sql = $"DELETE FROM InboxItems WHERE {where}";
    ...
    {
        string sql = @"SELECT FromContactId,
                    COUNT(*) as Total,
    ...
    {
        string sql = "UPDATE InboxItems SET PayloadJson=@payloadJson WHERE ID=@id";
        int affected = await dbConnection.ExecuteAsync(sql, new { id = inboxItemId, payloadJson });
        return affected > 0;
    }
}
