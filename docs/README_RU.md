...
- **Очистка текста и объединение сообщений**: Автоматическое удаление «мусорного» текста до ссылки, объединение текста, присланного отдельными сообщениями после ссылки, в одну подпись. Возможность перезаписывать подпись отдельным сообщением.
- **Inbox (входящие)**: Опциональное сохранение поступающего контента в БД для последующего просмотра. Доступен список, подробный просмотр и удаление.

### Очистка текста и объединение сообщений

Бот автоматически извлекает первую валидную ссылку из сообщения, удаляет «мусорный» текст, который некоторые сервисы добавляют до ссылки, и объединяет текст, отправленный отдельными сообщениями незадолго после ссылки, в одну подпись к медиа.

### Inbox (входящие)

Inbox позволяет сохранять входящий контент в базу данных вместо немедленной отправки получателю. Можно просматривать список входящих, открывать присланный с подробностями (отправитель, время, хэштеги, оригинальная подпись) и удалять элементы.

Как включить и использовать:
- Включается через Настройки → Приватность | безопасность → Inbox.
- Откройте Inbox из главного меню, листайте страницы, просматривайте элементы и удаляйте ненужные.
...
## Логирование
Бот логирует все действия в консоль. В будущем планируется добавление логирования в файл. Уровень логирования можно менять на лету через конфиг:
- `ConsoleOutputSettings:LogLevel` (или ENV `ConsoleOutputSettings__LogLevel`)
- Изменение применяется на лету; в логе появится строка вроде: `Applied hot config [ConsoleOutputSettings]...`.

### Конфигурация и ENV (кратко)
- Основной конфиг: `appsettings.json` (+ пример `appsettings.json.example`)
- Доп. конфиг загрузчиков: `downloader-config.json` (путь задаётся `AppSettings:DownloaderSettings:ConfigFilePath`)
- Переменные окружения имеют приоритет; формат ключей: `Section__Key` (двойное подчёркивание)
- Горячо применяются: LogLevel, Tor (Enabled/Interval/порты/хост/пароль), задержки, Proxy, AccessPolicy, параметры загрузчиков
- Требуют рестарта: токен бота, тип БД/строка подключения, смена пути к файлу конфигурации загрузчиков
