...
public sealed class DefaultSummaryService : IDefaultSummaryService
{
    ...
    public async Task<string> BuildDefaultsSummary(Update update)
    {
        try
        {
            ...
            List<int> users = _defaultGetter.GetAllDefaultUsersActionTargets(userId, TargetTypes.USER, actionId);
            List<int> groups = _defaultGetter.GetAllDefaultUsersActionTargets(userId, TargetTypes.GROUP, actionId);
