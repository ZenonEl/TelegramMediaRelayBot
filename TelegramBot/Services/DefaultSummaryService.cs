...
public sealed class DefaultSummaryService : IDefaultSummaryService
{
	...
	public async Task<string> BuildDefaultsSummary(Update update)
	{
		try
		{
			...
			return $"<b>{Users.GetResourceString("Summary.Defaults.Header")}</b>\n" +
			       $"{Users.GetResourceString("Summary.Defaults.Action")}: <code>{System.Net.WebUtility.HtmlEncode(action)}</code>\n" +
			       $"{Users.GetResourceString("Summary.Defaults.Timeout")}: <code>{System.Net.WebUtility.HtmlEncode(condition)}</code>\n" +
	...
	public Task<string> BuildTimeoutSummary(Update update)
	{
		try
		{
			...
			return Task.FromResult($"{Users.GetResourceString("Summary.Defaults.Timeout")}: <code>{System.Net.WebUtility.HtmlEncode(condition)}</code>\n\n");
		}
	...
	public Task<string> BuildPrivacySummary(Update update)
	{
		try
		{
			...
			if (_privacyGetter.GetIsActivePrivacyRule(userId, PrivacyRuleType.SOCIAL_SITE_FILTER)) enabled.Add(Users.GetResourceString("PrivacyFilter.Social"));
			if (_privacyGetter.GetIsActivePrivacyRule(userId, PrivacyRuleType.NSFW_SITE_FILTER)) enabled.Add(Users.GetResourceString("PrivacyFilter.NSFW"));
			if (_privacyGetter.GetIsActivePrivacyRule(userId, PrivacyRuleType.UNIFIED_SITE_FILTER)) enabled.Add(Users.GetResourceString("PrivacyFilter.Unified"));
			...
			string domainInfo = domainsOn ? Users.GetResourceString("DomainsFilterOn") : Users.GetResourceString("DomainsFilterOff");
			string inboxInfo = inboxOn ? Users.GetResourceString("InboxOn") : Users.GetResourceString("InboxOff");
			string preface = string.Format(Users.GetResourceString("PrivacyPrefaceTemplate"), string.Join(", ", enabled), domainInfo, inboxInfo);
