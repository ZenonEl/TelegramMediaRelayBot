...
public class ProcessContactState : IUserState
{
    ...
    public async Task ProcessState(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
    {
        ...
        switch (currentState)
        {
            case ContactState.WaitingForLink:
                ...
                await botClient.SendMessage(chatId, Localization.Get("UserFoundByLink"), cancellationToken: cancellationToken,
            ...
            case ContactState.WaitingForName:
                string text_data = $"{Localization.Get("LinkText")}: {link} \n{Localization.Get("NameText")}: {_userGetter.GetUserNameByID(_contactGetter.GetContactIDByLink(link))}";

                await botClient.SendMessage(chatId, Localization.Get("ConfirmAdditionText") + text_data, cancellationToken: cancellationToken,
                                            replyMarkup: ReplyKeyboardUtils.GetSingleButtonKeyboardMarkup(Localization.Get("NextButtonText")));
            ...
            case ContactState.WaitingForConfirmation:
                ...
                await botClient.SendMessage(chatId, Localization.Get("WaitForContactConfirmation"),
    ...
    public async Task SendNotification(ITelegramBotClient botClient, long chatId, CancellationToken cancellationToken)
    {
        await botClient.SendMessage(_userGetter.GetTelegramIDbyUserID(_contactGetter.GetContactIDByLink(link)), 
                                    string.Format(Localization.Get("UserWantsToAddYou"), _userGetter.GetUserNameByTelegramID(chatId)), 
                                    cancellationToken: cancellationToken);
    }
}